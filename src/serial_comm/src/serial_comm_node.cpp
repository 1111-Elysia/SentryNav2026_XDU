...
private:
    ...
    void cmdVelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
    {
        ...
        // 提取速度数据
        current_vx_ = static_cast<float>(msg->linear.x) / 50.0;   // 前后速度
        current_vy_ = static_cast<float>(msg->linear.y) / 50.0;   // 左右速度
        current_vyaw_ = static_cast<float>(msg->angular.z) / 50.0; // 旋转速度
