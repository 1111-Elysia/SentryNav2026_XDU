...
public:
                        ...
                        tf_listener_(tf_buffer_)
    {
        ...
        // ===== 新增：手动位置偏移参数 =====
        this->declare_parameter<bool>("use_manual_position", false);
        this->declare_parameter<double>("manual_position_x", -13.0);
        this->declare_parameter<double>("manual_position_y", -11.0);
        this->declare_parameter<double>("manual_position_z", 0.0);
        ...
        this->get_parameter("use_manual_position", use_manual_position_);
        this->get_parameter("manual_position_x", manual_position_x_);
        this->get_parameter("manual_position_y", manual_position_y_);
        this->get_parameter("manual_position_z", manual_position_z_);
        ...
        if (use_manual_position_) {
            RCLCPP_WARN(this->get_logger(), "  ⚠️ 使用手动位置模式");
            RCLCPP_INFO(this->get_logger(), "  手动位置: [%.2f, %.2f, %.2f]",
                manual_position_x_, manual_position_y_, manual_position_z_);
        } else {
    ...
    void timerCallback()
    {
        ...
        try {
            ...
            if (use_manual_position_) {
                ...
                map_to_odom.transform.translation.x = manual_position_x_;
                map_to_odom.transform.translation.y = manual_position_y_;
                map_to_odom.transform.translation.z = manual_position_z_;
                ...
                if (!manual_mode_logged_) {
                    RCLCPP_INFO(this->get_logger(), 
                        "手动模式: map→odom [%.2f, %.2f]",
                        manual_position_x_, manual_position_y_);
                    manual_mode_logged_ = true;
                }
                
            } else {
                // ===== 自动模式：从 Lightning-LM 获取 =====
                try {
                    ...
                    if (!warning_printed_ && (now - start_time_).seconds() > 5.0) {
                        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                            "等待 Lightning-LM 重定位... (提示: 可使用 use_manual_position:=true)");
                        warning_printed_ = true;
                    }
                }
            }
            
            // ===== 发布 odom→base_link (单位变换) =====
            ...
            if (has_relocalized && (now - last_status_log_time_).seconds() > 5.0) {
                RCLCPP_INFO(this->get_logger(),
                    "状态: %s | 速度: Fast-LIO%s | 位置: [%.2f, %.2f]",
                    use_manual_position_ ? "手动模式" : "重定位✓",
                    twist_received_ ? "✓" : "✗",
...
private:
    ...
    bool manual_mode_logged_ = false;
    ...
    // ===== 手动位置参数 =====
    bool use_manual_position_ = false;
    double manual_position_x_ = -13.0;
    double manual_position_y_ = -11.0;
    double manual_position_z_ = 0.0;
};
