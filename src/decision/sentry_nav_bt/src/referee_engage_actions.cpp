...
constexpr auto kRuneActivationDelayAfterYaw = std::chrono::milliseconds(3000);
...
BT::NodeStatus EngageRune::onStart()
{
    ...
    RCLCPP_INFO(
        node_->get_logger(),
        "EngageRune: 进入激活能量机关流程，等待按顺序发送 scan_mode=true -> yaw_controller=0 -> autoshoot=true -> yaw后延时3s -> 激活请求");
